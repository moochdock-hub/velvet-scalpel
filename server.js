        ...
        if (filePath) {
            ...
            if (fs.existsSync(resolved)) {
                ...
                if (contents.length > 0) {
                    ...
                    lastPromptSource = 'file';
                    lastPromptPath = resolved;
...
app.post('/admin/reload-prompt', (req, res) => {
    ...
    try {
        loadSystemPrompt();
        return res.json({ status: 'ok', message: 'System prompt reloaded', source: lastPromptSource, path: lastPromptPath });
    } catch (err) {
        return res.status(500).json({ error: 'Failed to reload prompt', details: err && err.message });
    }
});

// Admin endpoint: return a safe fingerprint + short preview of the active prompt.
const crypto = require('crypto');
app.get('/admin/prompt-fingerprint', (req, res) => {
    const adminKey = process.env.ADMIN_KEY;
    ...
    if (!adminKey && !allowedLocal) {
        return res.status(403).json({ error: 'Forbidden: access only allowed from localhost when ADMIN_KEY is not configured' });
    }

    try {
        const hash = crypto.createHash('sha256').update(String(SYSTEM_PROMPT)).digest('hex');
        const preview = String(SYSTEM_PROMPT).replace(/\s+/g, ' ').trim().slice(0, 200);
        return res.json({ source: lastPromptSource, path: lastPromptPath, sha256: hash, preview });
    } catch (err) {
        return res.status(500).json({ error: 'Failed to compute fingerprint', details: err && err.message });
    }
});
